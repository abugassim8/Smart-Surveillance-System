...
class FlatGui(QMainWindow):
    ...
    def getProfile(self,Name,screenshot,ui,uicounter,best_class_probability):
        ...
        best_class_probability = (best_class_probability * 100 )+ 30
        print('hell')
    ...
    def startClicked(self):
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                prevTime = 0
                self.ui = [Ui_Faceresult() for p in range(500)]
                uicounter = 0
                print('Yeah')
                while True:
    ...
    def train_image(self):
        ...
        datadir = './pre_img'
        modeldir = './model/20170511-185253.pb'  # for testing the 2018 .pb file
        classifier_filename = './class/classifier.pkl'
    ...
    def deleteData(self):
        ...
        if not user:
            self.warning2("Warning", "Please fill in the username and  password fields Again.. ")
        else:
            conn = sqlite3.connect('FaceNetdb.db')
            ...
            self.warning2("Warning", "Account Successfully deleted . ")
    ...
    def insertData(self):
        ...
        if not username:
            ...
            conn.commit()
            result = conn.execute("SELECT * FROM USERS")
            self.warning2("Warning", "Account Successfully Created. ")

    def showData(self):


        name = self.nameEdit_2.text()
        self.nameEdit_2.setClearButtonEnabled(True)

        conn = sqlite3.connect('FaceNetdb.db')


        cursor = conn.execute('''SELECT ID,NAME,GENDER,BIRTH_YEAR,RACE,CRIMINAL_RECORDS,PICTURE,Status
         FROM Suspects WHERE NAME = ?''', (name,))
    ...
    def insertNewData(self):
        ...
        picture_file = files
        pfile = self.folderEdit.text()
        print(pfile)

        if not pfile:
            self.warning2("Warning", "Please specify the Training Folder. ")
    ...
    def recorded_people(self):
        ...
        vfile = self.Video_Edit_2.text()
        if not vfile:
            self.warning2("Warning", "Please specify the video input. ")
        else:
    ...
    def reset_password(self):
        ...
        if len(usrname) < 2:
            self.warning2("Notification", "PLEASE ENTER USERNAME TO RESET ")
        else:
            password = "newpass"
            conn = sqlite3.connect('FaceNetdb.db')
            print("new password2")
            conn.execute("UPDATE USERS SET PASSWORD = ? WHERE USERNAME = ?", (password, usrname))
            print("new password3")
            conn.commit()
            self.warning2("Notification", "PASSWORD RESETED TO " + password)
